...
function toggleRoute(path: string, pageType = 'tab') {
  if (pageType === 'tab') {
    Taro.switchTab({
      url: path,
    });
...
@include b(portal-home) {
  ...
  @include e(tools-item) {
    color: #fff;
    background-image: linear-gradient(90deg, var(--app-primary-color) 0%, rgba(0, 0, 0, 0.01) 100%);
  }
  @include e(tools) {
    border-radius: 8px;
    background-color: #f5f5f5;
